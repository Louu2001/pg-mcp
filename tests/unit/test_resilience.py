...
class TestRetryAsync:
    ...
    async def test_succeeds_without_retry(self) -> None:
        """Successful first attempt should not sleep or retry."""
        calls = {"n": 0}
    ...
    async def test_retries_retryable_errors(self) -> None:
        """Retryable errors are retried until success."""
        calls = {"n": 0}
    ...
    async def test_raises_after_exhausting_attempts(self) -> None:
        """Last retryable error is re-raised when attempts are exhausted."""

...
class TestSQLGenerationWithRetry:
    ...
    @pytest.mark.asyncio
    async def test_blocked_table_does_not_retry(self, mock_schema: DatabaseSchema) -> None:
        """Access to blocked tables must fail immediately, not be rewritten."""
        mock_generator = AsyncMock()
        mock_generator.generate.return_value = "SELECT COUNT(*) FROM users AS u;"

        mock_validator = MagicMock()
        mock_validator.blocked_tables = {"users"}
        mock_validator.blocked_columns = set()
        mock_validator.validate_or_raise.side_effect = SecurityViolationError(
            "Access to table 'users' is not allowed"
        )
...
class TestResultValidation:
    ...
    async def test_validate_results_failure_does_not_raise(self) -> None:
        ...
        # Should not raise; unknown quality is reported as 0 rather than 100
        confidence = await orchestrator._validate_results_safely(
            question="Count users",
            sql="SELECT COUNT(*) FROM users",
            results=[{"count": 42}],
...
class TestExecuteQueryFlow:
    ...
    @pytest.mark.asyncio
    async def test_execute_query_rejects_write_intent(self, mock_schema: DatabaseSchema) -> None:
        """Natural-language deletes must fail closed without calling the LLM."""
        mock_generator = AsyncMock()
        mock_cache = MagicMock()
        mock_cache.get.return_value = mock_schema
        orchestrator = QueryOrchestrator(
            sql_generator=mock_generator,
            sql_validator=SQLValidator(config=SecurityConfig(allow_write_operations=False)),
            sql_executor=MagicMock(),
            result_validator=MagicMock(),
            schema_cache=mock_cache,
            pools={"test_db": MagicMock()},
        ...
        response = await orchestrator.execute_query(
            QueryRequest(question="删除所有用户", database="test_db")
        )

        assert response.success is False
        assert response.error is not None
        assert response.error.code == "security_violation"
        assert "不支持数据修改" in response.error.message
        assert response.error.details is not None
        assert response.error.details["blocked_operation"] == "DELETE"
...
class TestMultiDatabaseRouting:
    """Queries must run on the executor for the resolved database."""

    @pytest.mark.asyncio
    async def test_execute_uses_requested_database_executor(self) -> None:
        """Selecting database B must not execute on database A's executor."""
        schema = DatabaseSchema(database_name="db_b", tables=[], version="15.0")
        mock_cache = MagicMock()
        mock_cache.get.return_value = schema

        mock_generator = AsyncMock()
        mock_generator.generate.return_value = "SELECT 1;"
        mock_generator.last_tokens_used = 12
        ...
        executor_b.execute.return_value = ([{"n": 1}], 1)
        ...
        response = await orchestrator.execute_query(
            QueryRequest(question="count rows", database="db_b", return_type=ReturnType.RESULT)
        )

        assert response.success is True
        executor_b.execute.assert_awaited_once()
        executor_a.execute.assert_not_called()
        mock_cache.get.assert_called_once_with("db_b")

    @pytest.mark.asyncio
    async def test_question_too_long_rejected(self) -> None:
        """Questions longer than ValidationConfig.max_question_length are rejected."""
        orchestrator = QueryOrchestrator(
            sql_generator=MagicMock(),
            sql_validator=MagicMock(),
            sql_executor=MagicMock(),
            ...
            pools={"db": MagicMock()},
            resilience_config=ResilienceConfig(),
            validation_config=ValidationConfig(max_question_length=20),
        )

        response = await orchestrator.execute_query(
            QueryRequest(question="x" * 21, database="db", return_type=ReturnType.SQL)
        )
    ...
    @pytest.mark.asyncio
    async def test_result_validation_failure_uses_zero_confidence(
        self,
    ) -> None:
        """LLM result-validation errors must not report confidence 100."""
        schema = DatabaseSchema(database_name="db", tables=[], version="15.0")
        mock_cache = MagicMock()
        mock_cache.get.return_value = schema
        ...
        mock_executor = AsyncMock()
        mock_executor.execute.return_value = ([{"n": 1}], 1)

        mock_result_validator = AsyncMock()
        mock_result_validator.validate.side_effect = RuntimeError("llm down")
        ...
        response = await orchestrator.execute_query(
            QueryRequest(question="count", database="db", return_type=ReturnType.RESULT)
        )

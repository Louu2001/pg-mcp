...
class TestSQLGenerator:
    ...
    async def test_generate_simple_query(
        ...
        with patch.object(
            ...
            # Verify result
            assert result == "SELECT * FROM users;"
            assert generator.last_tokens_used == 0

    @pytest.mark.asyncio
    async def test_gpt5_uses_max_completion_tokens(
        self, mock_schema: DatabaseSchema
    ) -> None:
        """gpt-5.x models should use max_completion_tokens and omit temperature."""
        generator = SQLGenerator(
            OpenAIConfig(api_key=SecretStr("sk-test-key-12345"), model="gpt-5.5")
        )
        mock_response = MagicMock()
        mock_response.choices = [
            MagicMock(message=MagicMock(content="```sql\nSELECT 1;\n```"))
        ...
        with patch.object(
            ...
            await generator.generate("count", mock_schema)
    ...
    async def test_generate_records_token_usage(
        ...
        """OpenAI usage.total_tokens should be stored on the generator."""
        mock_response = MagicMock()
        mock_response.choices = [
            MagicMock(message=MagicMock(content="```sql\nSELECT 1;\n```"))
        ]
        ...
        with patch.object(
            ...
            result = await generator.generate("count", mock_schema)

        assert result == "SELECT 1;"
        assert generator.last_tokens_used == 42
    ...
    @pytest.mark.asyncio
    async def test_generate_handles_permission_denied(
        self, generator: SQLGenerator, mock_schema: DatabaseSchema
    ) -> None:
        """Quota/subscription errors should surface as LLMUnavailableError."""
        from openai import PermissionDeniedError

        err = PermissionDeniedError(
            message="no active subscription",
            response=MagicMock(status_code=403, headers={}),
            body={"error": {"message": "no active subscription"}},
        )
        with patch.object(
            generator.client.chat.completions,
            "create",
            ...
            with pytest.raises(LLMUnavailableError, match="余额不足"):
                await generator.generate("Count users", mock_schema)

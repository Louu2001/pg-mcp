...
class SQLValidator:
    ...
    - Rejecting natural-language write/DDL intent before SQL generation
    ...
    WRITE_INTENT_MESSAGE = "该操作被拒绝。本服务仅支持数据查询操作，不支持数据修改。"
    WRITE_INTENT_SUGGESTION = "如需修改数据，请联系数据库管理员或使用其他授权工具。"

    _SQL_LEAD: ClassVar[re.Pattern[str]] = re.compile(
        r"^\s*(?:"
        r"SELECT\b|WITH\b|EXPLAIN\b|"
        r"INSERT\s+INTO\b|UPDATE\s+\w+\s+SET\b|DELETE\s+FROM\b|"
        r"DROP\s+(?:TABLE|DATABASE|SCHEMA|INDEX|VIEW)\b|"
        r"TRUNCATE(?:\s+TABLE)?\s+\w+|"
        r"ALTER\s+TABLE\b|CREATE\s+(?:TABLE|INDEX|DATABASE|VIEW)\b|"
        r"GRANT\b|REVOKE\b|COPY\b|CALL\b|VACUUM\b|MERGE\s+INTO\b"
        r")",
        re.IGNORECASE,
    )

    _NL_WRITE_RULES: ClassVar[list[tuple[re.Pattern[str], str]]] = [
        (re.compile(r"(删除|删掉|移除|去掉)\s*(所有|全部)"), "DELETE"),
        (re.compile(r"^(请)?(帮我)?(删除|删掉|移除|清空)"), "DELETE"),
        (re.compile(r"(把|将).{1,80}(改成|改为|换成|修改为)"), "UPDATE"),
        (re.compile(r"^(请)?(帮我)?(修改|更改|更新)"), "UPDATE"),
        (re.compile(r"(更新|修改)\s*(所有|全部)"), "UPDATE"),
        (re.compile(r"清空\s*(所有|全部|表|数据)?"), "TRUNCATE"),
        (re.compile(r"(插入|新增(一条|一个|用户)|添加(一条|一个|用户))"), "INSERT"),
        (re.compile(r"(删除表|删表)"), "DROP"),
        (re.compile(r"\bdelete\s+(all|from|the)\b", re.IGNORECASE), "DELETE"),
        (re.compile(r"\bdrop\s+(table|database|schema|index|view)\b", re.IGNORECASE), "DROP"),
        ...
        (re.compile(r"\b(remove|erase)\s+all\b", re.IGNORECASE), "DELETE"),
    ]
    ...
    def __init__(
        ...
        self.blocked_tables = {t.lower() for t in tables}
        self.blocked_columns = {c.lower() for c in columns}
    ...
    def check_question_intent(self, question: str) -> None:
        """Reject natural-language or raw-SQL write/DDL requests.

        Args:
            question: User question, which may be natural language or pasted SQL.

        Raises:
            SecurityViolationError: If the question asks to modify data and
                write operations are not allowed.
        """
        if self.config.allow_write_operations:
            return
        ...
        if self._SQL_LEAD.search(text):
            try:
                self.validate_or_raise(text)
            except SQLParseError:
                raise SecurityViolationError(
                    self.WRITE_INTENT_MESSAGE,
                    details=self._write_intent_details("SQL"),
                ) from None
            return

        for pattern, operation in self._NL_WRITE_RULES:
            if pattern.search(text):
                raise SecurityViolationError(
                    self.WRITE_INTENT_MESSAGE,
                    details=self._write_intent_details(operation),
                )

    def _write_intent_details(self, operation: str) -> dict[str, str]:
        return {
            "blocked_operation": operation,
            "suggestion": self.WRITE_INTENT_SUGGESTION,
        }
